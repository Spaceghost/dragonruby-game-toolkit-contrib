...
pub fn build(b: *std.Build) void {
    ...
    b.step("check", "Differentially validate update, pack and one-call sink stages").dependOn(&check.step);

    const run = b.addRunArtifact(exe);
    if (b.args) |args| run.addArgs(args);
    run.has_side_effects = true;
    b.step("run", "Benchmark persistent starfield stages").dependOn(&run.step);
}
