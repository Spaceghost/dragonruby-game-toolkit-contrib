...
static uint64_t batch(const bench_case *c, unsigned variant, size_t n) {
    ...
    /* A formula independent of either implementation verifies every batch. */
    const uint64_t expected_per_call[] = {8, 2080, 255, 4090, 1, 1};
...
#endif
    printf("{\"event\":\"ruby_environment\",\"version\":\"%s\",\"boxing\":\"%s\",\"gc\":\"enabled\",\"renderer\":false,\"adapter_linkage\":\"static-test-host\",\"live_units\":\"requested-payload-bytes\"}\n", MRUBY_VERSION, DRBZ_BOXING_NAME);
