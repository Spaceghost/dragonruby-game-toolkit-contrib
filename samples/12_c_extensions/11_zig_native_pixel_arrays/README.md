...
These layers are intentionally separate. A passing mruby harness is not a claim
about the proprietary DragonRuby API table layout or renderer.
...
`.github/workflows/zig-mruby.yml` builds both pinned upstream mruby 3.4.0 and
mruby 3.0.0 with the complete published `docs/dragonruby-mruby.patch`. Each source
is tested in word-boxed, NaN-boxed and unboxed configurations. The host
dynamically loads the **actual `app/bridge.c` plus Zig** and runs
...
The published-patch target verifies the exact base commit and patch blob before
applying the patch unchanged. It also checks the patched VM's cached symbols
and floating-point integer-division behavior inside the same C/Zig integration
host. This prevents an accidentally unpatched interpreter from passing that
job. Changes to the published patch trigger CI and require review of its pin.
See [the compatibility target's inputs and reproduction steps](tests/DRAGONRUBY_MRUBY.md).
...
test source as a pass. Testing the published patch does not establish the
configuration or compatibility of a current proprietary SDK.
...
four targets. mruby integration uses six standard Ubuntu jobs: two source
variants times three value layouts. Cross-building is distinct from native
execution, and neither replaces the real SDK test.
